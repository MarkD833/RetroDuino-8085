...
OUT:
	PUSH	PSW		; Save char
OUT1:
	IN	01h			; Get UART status
	ANI 00000100b	; Test for TxRDY = 1
	JZ	OUT1		; Not ready - try again
	POP	PSW			; Restore char
	OUT	03h			; Write char
	RET
	
; Check for a character from the console (SCC2691 UART)
IN:	IN	01h			; Get UART status
	ANI	00000001b	; Test for RxRDY bit = 1
	RZ				; No char
	IN	03h			; Get received data
	RET
;
; INIT is called from the main mointor code to initialize the
; specific hardware of the board
;
INIT:
;
...
;
	...
	MVI	A,00010011b	; 8 DATA BITS + NO PARITY 
	OUT 00h			; write to MODE REGISTER #1
	MVI	A,00000111b	; 1 STOP BIT 
	OUT 00h			; write to MODE REGISTER #2
	MVI	A,10111011b	; 9600 BAUD for RX & TX 
	OUT 01h			; write to CLOCK SELECT REGISTER
	MVI A,0			; no interrupts
	OUT 05h			; write to INTERRUPT MASK REGISTER
...
; Copy the whole of MON85 from ROM to RAM
; On power-up, memory is configured like this:
; 0x0000 .. 0x3FFF : READ from ROM + WRITE TO RAM
; 040000 .. 0xFFFF : READ from RAM + WRITE to RAM
;
; Once MON85 is copied from ROM to RAM, then ROM is
; disabled and 0x0000 .. 0xFFFF is all RAM READ & WRITE
;
INIT0:
    LXI D,ROMEND    ; load the address of the last byte of MON85
    INR D           ; add 1 to upper 8 bits for the counter to work!
					; DE holds the number of bytes to copy
					
    LXI B,00h       ; BC holds the start address = 0x0000
INIT1:
    LDAX B          ; read byte from ROM
    STAX B          ; write it to RAM (same address)
...
; The MON85 code should now all reside in RAM as well as ROM
; so disable the ROM by setting SOD to a 1
...
PRECOPY:
	DB	0Dh,0Ah
	DB	"Copying ROM to RAM",0Dh,0Ah,0
POSTCOPY:
    DB  "Copy complete - ROM Disabled.",0Dh,0Ah;
	DB	0Dh,0Ah
	DB	"RetroDuino-8085 SCC2691 UART - RAM Monitor Variant.",0Dh,0Ah
	DB  "Full 64K RAM is accessible - Monitor uses 0x0000-0x1100."
