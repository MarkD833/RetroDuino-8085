...
* [Software](#software)
  ...
  - [Simple LED flasher #1](#a-simple-led-flasher)
  - [Simple LED flasher #2](#a-simple-led-flasher-in-ram)
...
# Pinout
Below is an image of the pinout I'm using - laid out in a similar way to the Arduino pinout diagrams found on the web. Note that the I2C and SPI signals will hopefully be generated by a software implementation of a hardware interface. 
...
The LEDTest2 folder contains the same simple LED flashing code as above, BUT with an interesting feature. Note that this code should only be used with the MON85 ROM monitor - it won't work with the MON85 RAM monitor.

The MON85 monitor resides in ROM starting at address 0x0000. The downside of this is usually that you cannot put your own interrupt handlers there as it's Read Only. However, with this design, you can!

The LEDTest2 code is split across 3 areas of memory.
...
Finally there is a simple few bytes of code that are loaded high up in RAM at address 0xFF00. This simple bit of code sets the 8085 SOD pin high - switching out the MON85 ROM completely - and then jumping to address 0x0000 to execute whatever code it finds there.

The code is executed by giving MON85 the command "g FF00".
